...
// void remmina_rdp_cliprdr_request_data(GdkClipboard *GdkClipboard, GtkSelectionData *selection_data, guint info, RemminaProtocolWidget *gp)
// {
// 	TRACE_CALL(__func__);
...
// 		gtk_clipboard_set_with_owner(GdkClipboard, targets, n_targets,
// 							(GdkClipboardGetFunc)remmina_rdp_cliprdr_request_data,
// 							(GdkClipboardClearFunc)remmina_rdp_cliprdr_empty_clipboard, G_OBJECT(gp));
...
// 	GdkClipboard = gtk_widget_get_clipboard(rfi->drawing_area, GDK_SELECTION_CLIPBOARD);
// 	if (GdkClipboard && gtk_clipboard_get_owner(GdkClipboard) == (GObject *)gp)
// 		gtk_clipboard_clear(GdkClipboard);
