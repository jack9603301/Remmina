...
void rcw_toolbar_fullscreen_option(GtkWidget *toggle, RemminaConnectionWindow *cnnwin)
{
	...
	menuitem = gtk_check_button_new_with_label( _("Viewport fullscreen mode"));
...
static void rcw_toolbar_scaler_option(GtkWidget *toggle, RemminaConnectionWindow *cnnwin)
{
	...
	menuitem = gtk_button_new_with_label(_("Keep aspect ratio when scaled"));
	gtk_widget_show(menuitem);
	//gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
	//group = gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(menuitem));
	if (!scaler_expand)
	...
	//menuitem = gtk_radio_menu_item_new_with_label(group, _("Fill client window when scaled")); TODO GTK4
	gtk_widget_show(menuitem);
	//gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
	if (scaler_expand)
		gtk_check_button_set_active(GTK_CHECK_BUTTON(menuitem), TRUE);
...
static void rco_call_protocol_feature_radio(GtkButton *menuitem, RemminaConnectionObject *cnnobj)
{
	TRACE_CALL(__func__);
				   ...
				   GtkWidget *menu, const RemminaProtocolFeature *feature, const gchar *domain, gboolean enabled)
{
	...
	for (i = 0; list[i]; i += 2) {
		menuitem = gtk_button_new_with_label(g_dgettext(domain, list[i + 1]));
		//group = gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(menuitem));
		gtk_widget_show(menuitem);
		//gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
				   ...
				   const gchar *domain, gboolean enabled)
{
	...
	menuitem = gtk_check_button_new_with_label(g_dgettext(domain, (const gchar *)feature->opt3));
	gtk_widget_show(menuitem);
	//gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
	...
	if (enabled) {
		...
		gtk_toggle_button_set_active(GTK_CHECK_BUTTON(menuitem),
...
rcw_create_toolbar(RemminaConnectionWindow *cnnwin, gint mode)
{
	...
	//GTK_BOX_set_style(GTK_BOX(toolbar), GTK_BOX_ICONS);
...
#endif
	gtk_box_append(GTK_BOX(toolitem), widget);
	...
	if (remmina_pref.small_toolbutton)
		gtk_widget_set_name(widget, "remmina-small-button");
	gtk_box_append(toolitem, widget);
#if GTK_CHECK_VERSION(3, 14, 0)
	//arrow = gtk_image_new_from_icon_name("org.remmina.Remmina-pan-down-symbolic", GTK_ICON_SIZE_SMALL_TOOLBAR);
#else
	arrow = gtk_arrow_new(GTK_ARROW_DOWN, GTK_SHADOW_NONE);
...
static gboolean rcw_floating_toolbar_on_leave(GtkWidget *widget, GdkCrossingEvent *event,
					      RemminaConnectionWindow *cnnwin)
{
	TRACE_CALL(__func__);
...
static gboolean rcw_on_leave_notify_event(GtkWidget *widget, GdkCrossingEvent *event,
					  gpointer user_data)
{
	TRACE_CALL(__func__);
...
static gboolean rco_leave_protocol_widget(GtkWidget *widget, GdkCrossingEvent *event,
					  RemminaConnectionObject *cnnobj)
{
	TRACE_CALL(__func__);
...
gboolean rco_enter_protocol_widget(GtkWidget *widget, GdkCrossingEvent *event,
				   RemminaConnectionObject *cnnobj)
{
	TRACE_CALL(__func__);
	gboolean active;
...
static gboolean rcw_floating_toolbar_on_scroll(GtkWidget *widget, GdkScrollEvent *event,
					       RemminaConnectionWindow *cnnwin)
{
	TRACE_CALL(__func__);
	RemminaConnectionObject *cnnobj;
...
static GtkWidget *rco_create_tab_label(RemminaConnectionObject *cnnobj)
{
	...
	gtk_widget_show(widget);
	gtk_box_append(GTK_BOX(button), widget);
...
static RemminaConnectionWindow *rcw_create_scrolled(gint width, gint height, gboolean maximize)
{
	...
	gtk_box_append(GTK_BOX(cnnwin), grid);
...
static void rcw_create_overlay_ftb_overlay(RemminaConnectionWindow *cnnwin)
{
	...
	gtk_box_append(GTK_BOX(revealer), priv->floating_toolbar_widget);
	...
	gtk_box_append(GTK_BOX(fr), vbox);
...
static gboolean rcw_hostkey_func(RemminaProtocolWidget *gp, guint keyval, gboolean release)
{
	...
	if (keyval == remmina_pref.shortcutkey_fullscreen && !extrahardening) {
		...
		if (priv->toolitem_autofit && gtk_widget_is_sensitive(GTK_WIDGET(priv->toolitem_autofit)))
			rcw_toolbar_autofit(GTK_BUTTON(gp), cnnobj->cnnwin);
		...
		rcw_toolbar_minimize(GTK_BUTTON(gp),
				     cnnobj->cnnwin);
		...
		rcw_toolbar_screenshot(GTK_BUTTON(gp),
				       cnnobj->cnnwin);
